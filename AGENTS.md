...
> **Tier: MVP** — speed over completeness; global minimum standards apply (see `~/.claude/CLAUDE.md`).

Daily golf/putting news research system for the Putting Lab project, publishing digests to news.putting-lab.ch. A Next.js 16 site (`src/`) renders the markdown digests in `news/` (daily) and `deep-dives/` (analyses). Key commands: `npm run dev` / `npm run build` / `npm run lint`, plus `npm run validate-digest` (lint a digest against the format rules in `scripts/validate-digest.mjs`) and `npm run test:digest`.

The rules below are the content ruleset for the news agent. They are intentionally in German (the output language) and validated by scripts/validate-digest.mjs — do not translate or reword them.
